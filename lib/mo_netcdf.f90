...
module mo_netcdf
  ...
  !
  !     PURPOSE
  !>        \brief Holds the basic file wide modification functionality
  ...
  !>                 All files created by this derived types and its procedures are
  !>                 of type NF90_NETCDF4
  ...
  type NcDataset
   ...
   contains
     ...
     ! 
     !     PURPOSE
     !>        \brief Checks if dimension exists.
     ...
     ! 
     !     PURPOSE
     !>        \brief Returns the unlimited dimension of the dataset
     !>        \details Returns the NcDimension type of the unlimited dimension. 
     ...
     ! 
     !     PURPOSE
     !>        \brief Checks if the dataset is unlimited
     ...
     !>                 no the file cannot be closed correctly.
     !
     !     INTENT(IN)
     !         None
     !
     ...
     !>        \details Create a new dimension of given length. A length < 0 makes the
     !>                 dimension unlimited. The program will teminate abruptly if the
     ...
     !>                 A creation error will result in abrupt program termination.
     !
  ...
  !>                 getDimension/setDimension functions of a NcDataset instance should
  ...
  type NcDimension
   ...
   contains
     ...
     ! 
     !     PURPOSE
     !>        \brief Checks if the dimension is unlimited
  ...
  !
  !     PURPOSE
  !>        \brief Provides the variable releated write/read functionality
  !>        \details Bound to this derived type is the main variable related NetCDF
  !>                 functionality, i.e. writing/reading data and attributes, as well
  ...
  type NcVariable
   ...
   contains
     ...
     !
     !     PURPOSE
     !>        \brief Retrieve the number of dimension 
     ...
     !>        \details Return the encoded data type of the variables
     ...
     !
     !     PURPOSE
     !>        \brief Checks if attribute exists.
     ...
     ! 
     !     PURPOSE
     !>        \brief Checks if the variable is unlimited
     !>        \details Returns true if the variables is associated to an unlimited 
     ...
     ! 
     !     PURPOSE
     !>        \brief Write given data
     !>        \details Writes the given data into the variable at an optionally given
     !>                 position.
     ...
     !     PURPOSE
     !>        \brief Retrieve data
     !>        \details Read the given data from an optionally given position.
     ...
     !>        \details Retrieve the variable fill value or a default value if not
     !>                 explicitly set.

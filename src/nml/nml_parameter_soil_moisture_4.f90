!> \file nml_parameter_soil_moisture_4.f90
!> \copydoc nml_soil_moisture_4

!> \brief Soil moisture - Case 4
!> \details Feddes ET reduction with global field-capacity dependency on root fraction.
!> \version 0.2
...
module nml_soil_moisture_4
  ...
  !> \class nml_soil_moisture_4_t
  !> \brief Soil moisture - Case 4
  !> \details Feddes ET reduction with global field-capacity dependency on root fraction.
  type, public :: nml_soil_moisture_4_t
  ...
  contains
    procedure :: init => nml_soil_moisture_4_init
    procedure :: init_type => nml_soil_moisture_4_init_type
    procedure :: from_file => nml_soil_moisture_4_from_file
    procedure :: set => nml_soil_moisture_4_set
    procedure :: is_set => nml_soil_moisture_4_is_set
    procedure :: is_valid => nml_soil_moisture_4_is_valid
  end type nml_soil_moisture_4_t

contains

  !> \brief Initialize defaults and sentinels for soil_moisture_4
  integer function nml_soil_moisture_4_init(this, errmsg) result(status)
    class(nml_soil_moisture_4_t), intent(inout) :: this !< namelist instance
  ...
  end function nml_soil_moisture_4_init

  !> \brief Initialize derived values with their field-specific defaults
  integer function nml_soil_moisture_4_init_type(this, &
    ...
    class(nml_soil_moisture_4_t), intent(in) :: this !< parent namelist instance
  ...
  end function nml_soil_moisture_4_init_type


  !> \brief Read soil_moisture_4 namelist from file
  integer function nml_soil_moisture_4_from_file(this, file, errmsg) result(status)
    class(nml_soil_moisture_4_t), intent(inout) :: this !< namelist instance
    ...
    status = nml%find("soil_moisture_4", errmsg=errmsg)
    ...
    ! read namelist
    read(nml%unit, nml=soil_moisture_4, iostat=iostat, iomsg=iomsg)
    if (iostat /= 0) then
      status = NML_ERR_READ
    ...
    status = NML_OK
  end function nml_soil_moisture_4_from_file

  !> \brief Set soil_moisture_4 values
  integer function nml_soil_moisture_4_set(this, &
    organic_matter_forest, &
    organic_matter_impervious, &
    organic_matter_pervious, &
    ptf_lower_66_5_constant, &
    ...
    this%is_configured = .true.
    status = NML_OK
  end function nml_soil_moisture_4_set

  !> \brief Check whether a namelist value was set
  integer function nml_soil_moisture_4_is_set(this, name, idx, errmsg) result(status)
    class(nml_soil_moisture_4_t), intent(in) :: this !< namelist instance
  ...
  end function nml_soil_moisture_4_is_set

  !> \brief Validate required values and constraints
  integer function nml_soil_moisture_4_is_valid(this, errmsg) result(status)
    class(nml_soil_moisture_4_t), intent(in) :: this !< namelist instance
    character(len=*), intent(out), optional :: errmsg !< error message for non-OK status values
    integer :: istat
  ...
  end function nml_soil_moisture_4_is_valid

end module nml_soil_moisture_4

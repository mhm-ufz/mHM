!> \file nml_output_mlm.f90
!> \copydoc nml_output_mlm

!> \brief mLM output configuration
!> \details Output configuration for mLM lake points.
!> \version 0.2
...
module nml_output_mlm
  ...
  logical, parameter, public :: out_lake_outflow__default = .false.
  ...
  !> \class nml_output_mlm_t
  !> \brief mLM output configuration
  !> \details Output configuration for mLM lake points.
  type, public :: nml_output_mlm_t
    ...
    logical :: out_lake_outflow !< Lake outflow
  contains
    procedure :: init => nml_output_mlm_init
    procedure :: from_file => nml_output_mlm_from_file
    procedure :: set => nml_output_mlm_set
    procedure :: is_set => nml_output_mlm_is_set
    procedure :: is_valid => nml_output_mlm_is_valid
  end type nml_output_mlm_t
...
contains
  ...
  !> \brief Initialize defaults and sentinels for output_mlm
  integer function nml_output_mlm_init(this, errmsg) result(status)
    class(nml_output_mlm_t), intent(inout) :: this !< namelist instance
    ...
    this%out_lake_outflow = out_lake_outflow__default ! bool values always need a default
  end function nml_output_mlm_init


  !> \brief Read output_mlm namelist from file
  integer function nml_output_mlm_from_file(this, file, errmsg) result(status)
    class(nml_output_mlm_t), intent(inout) :: this !< namelist instance
    ...
    out_lake_outflow = this%out_lake_outflow

    status = nml%open(file, errmsg=errmsg)
    if (status /= NML_OK) return

    status = nml%find("output_mlm", errmsg=errmsg)
    ...
    ! read namelist
    read(nml%unit, nml=output_mlm, iostat=iostat, iomsg=iomsg)
    if (iostat /= 0) then
      status = NML_ERR_READ
    ...
    this%out_lake_outflow = out_lake_outflow

    ! mark as configured
    this%is_configured = .true.
    status = NML_OK
  end function nml_output_mlm_from_file

  !> \brief Set output_mlm values
  integer function nml_output_mlm_set(this, &
    output_deflate_level, &
    output_double_precision, &
    output_time_reference, &
    output_frequency, &
    ...
    logical, intent(in), optional :: out_lake_outflow !< Lake outflow
    ...
    if (present(out_lake_outflow)) this%out_lake_outflow = out_lake_outflow

    ! mark as configured
    this%is_configured = .true.
    status = NML_OK
  end function nml_output_mlm_set

  !> \brief Check whether a namelist value was set
  integer function nml_output_mlm_is_set(this, name, idx, errmsg) result(status)
    class(nml_output_mlm_t), intent(in) :: this !< namelist instance
    ...
    case ("out_lake_outflow")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'out_lake_outflow'"
        return
      end if
    case default
  ...
  end function nml_output_mlm_is_set

  !> \brief Validate required values and constraints
  integer function nml_output_mlm_is_valid(this, errmsg) result(status)
    class(nml_output_mlm_t), intent(in) :: this !< namelist instance
    character(len=*), intent(out), optional :: errmsg !< error message for non-OK status values
    integer :: istat
  ...
  end function nml_output_mlm_is_valid

end module nml_output_mlm

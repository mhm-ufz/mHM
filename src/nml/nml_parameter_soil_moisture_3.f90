!> \file nml_parameter_soil_moisture_3.f90
!> \copydoc nml_soil_moisture_3

!> \brief Soil moisture - Case 3
!> \details Jarvis ET reduction with global field-capacity dependency on root fraction.
!> \version 0.2
...
module nml_soil_moisture_3
  ...
  !> \class nml_soil_moisture_3_t
  !> \brief Soil moisture - Case 3
  !> \details Jarvis ET reduction with global field-capacity dependency on root fraction.
  type, public :: nml_soil_moisture_3_t
    ...
    type(parameter_t) :: root_fraction_sand !< Sand contribution to the root-fraction coefficient
    type(parameter_t) :: root_fraction_clay !< Clay contribution to the root-fraction coefficient
    type(parameter_t) :: field_capacity_min !< Global minimum field capacity
    type(parameter_t) :: field_capacity_delta !< Global field-capacity range
    type(parameter_t) :: jarvis_sm_threshold !< Jarvis soil-moisture threshold
  contains
    procedure :: init => nml_soil_moisture_3_init
    procedure :: init_type => nml_soil_moisture_3_init_type
    procedure :: from_file => nml_soil_moisture_3_from_file
    procedure :: set => nml_soil_moisture_3_set
    procedure :: is_set => nml_soil_moisture_3_is_set
    procedure :: is_valid => nml_soil_moisture_3_is_valid
  end type nml_soil_moisture_3_t

contains

  !> \brief Initialize defaults and sentinels for soil_moisture_3
  integer function nml_soil_moisture_3_init(this, errmsg) result(status)
    class(nml_soil_moisture_3_t), intent(inout) :: this !< namelist instance
    ...
    status = this%init_type( &
      ...
      root_fraction_sand=this%root_fraction_sand, &
      root_fraction_clay=this%root_fraction_clay, &
      field_capacity_min=this%field_capacity_min, &
      field_capacity_delta=this%field_capacity_delta, &
      jarvis_sm_threshold=this%jarvis_sm_threshold, &
      errmsg=errmsg)
    if (status /= NML_OK) return
  end function nml_soil_moisture_3_init

  !> \brief Initialize derived values with their field-specific defaults
  integer function nml_soil_moisture_3_init_type(this, &
    ...
    class(nml_soil_moisture_3_t), intent(in) :: this !< parent namelist instance
    ...
    type(parameter_t), intent(inout), optional :: root_fraction_sand !< Sand contribution to the root-fraction coefficient
    type(parameter_t), intent(inout), optional :: root_fraction_clay !< Clay contribution to the root-fraction coefficient
    type(parameter_t), intent(inout), optional :: field_capacity_min !< Global minimum field capacity
    type(parameter_t), intent(inout), optional :: field_capacity_delta !< Global field-capacity range
    type(parameter_t), intent(inout), optional :: jarvis_sm_threshold !< Jarvis soil-moisture threshold
    character(len=*), intent(out), optional :: errmsg !< error message for non-OK status values
    ...
    if (present(organic_matter_forest)) then
      ...
      organic_matter_forest%lower_bound = 0.0_dp
      organic_matter_forest%upper_bound = 20.0_dp
    end if
    ...
    if (present(organic_matter_pervious)) then
      ...
      organic_matter_pervious%lower_bound = 0.0_dp
      organic_matter_pervious%upper_bound = 4.0_dp
    end if
    if (present(ptf_lower_66_5_constant)) then
      ...
      ptf_lower_66_5_constant%lower_bound = 0.6462_dp
      ptf_lower_66_5_constant%upper_bound = 0.9506_dp
    end if
    if (present(ptf_lower_66_5_clay)) then
      ...
      ptf_lower_66_5_clay%lower_bound = 0.0001_dp
      ptf_lower_66_5_clay%upper_bound = 0.0029_dp
    end if
    if (present(ptf_lower_66_5_bulk_density)) then
      ...
      ptf_lower_66_5_bulk_density%lower_bound = -0.3727_dp
      ptf_lower_66_5_bulk_density%upper_bound = -0.1871_dp
    end if
    if (present(ptf_upper_66_5_constant)) then
      ...
      ptf_upper_66_5_constant%lower_bound = 0.5358_dp
      ptf_upper_66_5_constant%upper_bound = 1.1232_dp
    end if
    if (present(ptf_upper_66_5_clay)) then
      ...
      ptf_upper_66_5_clay%lower_bound = -0.0055_dp
      ptf_upper_66_5_clay%upper_bound = 0.0049_dp
    end if
    if (present(ptf_upper_66_5_bulk_density)) then
      ...
      ptf_upper_66_5_bulk_density%lower_bound = -0.5513_dp
      ptf_upper_66_5_bulk_density%upper_bound = -0.0913_dp
    end if
    ...
    if (present(root_fraction_forest)) then
      ...
      root_fraction_forest%lower_bound = 0.97_dp
      root_fraction_forest%upper_bound = 0.985_dp
    end if
    if (present(root_fraction_impervious)) then
      ...
      root_fraction_impervious%lower_bound = 0.97_dp
      root_fraction_impervious%upper_bound = 0.985_dp
    end if
    if (present(root_fraction_pervious)) then
      ...
      root_fraction_pervious%lower_bound = 0.97_dp
      root_fraction_pervious%upper_bound = 0.985_dp
    end if
    ...
    if (present(root_fraction_sand)) then
      root_fraction_sand%value = ieee_value(root_fraction_sand%value, ieee_quiet_nan) ! sentinel for derived component value
      root_fraction_sand%optimize = .false.
      root_fraction_sand%lower_bound = ieee_value(root_fraction_sand%lower_bound, ieee_quiet_nan) ! sentinel for derived component lower_bound
      root_fraction_sand%upper_bound = ieee_value(root_fraction_sand%upper_bound, ieee_quiet_nan) ! sentinel for derived component upper_bound
      root_fraction_sand%lower_bound = 0.001_dp
      root_fraction_sand%upper_bound = 0.09_dp
    end if
    if (present(root_fraction_clay)) then
      root_fraction_clay%value = ieee_value(root_fraction_clay%value, ieee_quiet_nan) ! sentinel for derived component value
      root_fraction_clay%optimize = .false.
      root_fraction_clay%lower_bound = ieee_value(root_fraction_clay%lower_bound, ieee_quiet_nan) ! sentinel for derived component lower_bound
      root_fraction_clay%upper_bound = ieee_value(root_fraction_clay%upper_bound, ieee_quiet_nan) ! sentinel for derived component upper_bound
      root_fraction_clay%lower_bound = 0.9_dp
      root_fraction_clay%upper_bound = 0.999_dp
    end if
    if (present(field_capacity_min)) then
      field_capacity_min%value = ieee_value(field_capacity_min%value, ieee_quiet_nan) ! sentinel for derived component value
      field_capacity_min%optimize = .false.
      field_capacity_min%lower_bound = ieee_value(field_capacity_min%lower_bound, ieee_quiet_nan) ! sentinel for derived component lower_bound
      field_capacity_min%upper_bound = ieee_value(field_capacity_min%upper_bound, ieee_quiet_nan) ! sentinel for derived component upper_bound
      field_capacity_min%lower_bound = 0.1_dp
      field_capacity_min%upper_bound = 0.2_dp
    end if
    if (present(field_capacity_delta)) then
      field_capacity_delta%value = ieee_value(field_capacity_delta%value, ieee_quiet_nan) ! sentinel for derived component value
      field_capacity_delta%optimize = .false.
      field_capacity_delta%lower_bound = ieee_value(field_capacity_delta%lower_bound, ieee_quiet_nan) ! sentinel for derived component lower_bound
      field_capacity_delta%upper_bound = ieee_value(field_capacity_delta%upper_bound, ieee_quiet_nan) ! sentinel for derived component upper_bound
      field_capacity_delta%lower_bound = 0.1_dp
      field_capacity_delta%upper_bound = 0.4_dp
    end if
    if (present(jarvis_sm_threshold)) then
      jarvis_sm_threshold%value = ieee_value(jarvis_sm_threshold%value, ieee_quiet_nan) ! sentinel for derived component value
      jarvis_sm_threshold%optimize = .false.
      jarvis_sm_threshold%lower_bound = ieee_value(jarvis_sm_threshold%lower_bound, ieee_quiet_nan) ! sentinel for derived component lower_bound
      jarvis_sm_threshold%upper_bound = ieee_value(jarvis_sm_threshold%upper_bound, ieee_quiet_nan) ! sentinel for derived component upper_bound
      jarvis_sm_threshold%lower_bound = 0.0_dp
      jarvis_sm_threshold%upper_bound = 1.0_dp
    end if
  end function nml_soil_moisture_3_init_type


  !> \brief Read soil_moisture_3 namelist from file
  integer function nml_soil_moisture_3_from_file(this, file, errmsg) result(status)
    class(nml_soil_moisture_3_t), intent(inout) :: this !< namelist instance
    ...
    type(parameter_t) :: root_fraction_sand
    type(parameter_t) :: root_fraction_clay
    type(parameter_t) :: field_capacity_min
    type(parameter_t) :: field_capacity_delta
    type(parameter_t) :: jarvis_sm_threshold
    ! locals
    type(nml_file_t) :: nml
    integer :: iostat
    ...
    status = nml%find("soil_moisture_3", errmsg=errmsg)
    ...
    ! read namelist
    read(nml%unit, nml=soil_moisture_3, iostat=iostat, iomsg=iomsg)
    if (iostat /= 0) then
      status = NML_ERR_READ
    ...
    this%root_fraction_sand = root_fraction_sand
    this%root_fraction_clay = root_fraction_clay
    this%field_capacity_min = field_capacity_min
    this%field_capacity_delta = field_capacity_delta
    this%jarvis_sm_threshold = jarvis_sm_threshold

    ! mark as configured
    this%is_configured = .true.
    status = NML_OK
  end function nml_soil_moisture_3_from_file

  !> \brief Set soil_moisture_3 values
  integer function nml_soil_moisture_3_set(this, &
    organic_matter_forest, &
    organic_matter_impervious, &
    organic_matter_pervious, &
    ptf_lower_66_5_constant, &
    ...
    type(parameter_t), intent(in) :: root_fraction_sand !< Sand contribution to the root-fraction coefficient
    type(parameter_t), intent(in) :: root_fraction_clay !< Clay contribution to the root-fraction coefficient
    type(parameter_t), intent(in) :: field_capacity_min !< Global minimum field capacity
    type(parameter_t), intent(in) :: field_capacity_delta !< Global field-capacity range
    type(parameter_t), intent(in) :: jarvis_sm_threshold !< Jarvis soil-moisture threshold
    ...
    this%is_configured = .true.
    status = NML_OK
  end function nml_soil_moisture_3_set

  !> \brief Check whether a namelist value was set
  integer function nml_soil_moisture_3_is_set(this, name, idx, errmsg) result(status)
    class(nml_soil_moisture_3_t), intent(in) :: this !< namelist instance
    ...
    case ("root_fraction_sand%value")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_sand'"
        return
      end if
      if (ieee_is_nan(this%root_fraction_sand%value)) status = NML_ERR_NOT_SET
    case ("root_fraction_sand%optimize")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_sand'"
        return
      end if
    case ("root_fraction_sand%lower_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_sand'"
        return
      end if
    case ("root_fraction_sand%upper_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_sand'"
        return
      end if
    case ("root_fraction_sand")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_sand'"
        return
      end if
      if (ieee_is_nan(this%root_fraction_sand%value)) then
        status = NML_ERR_NOT_SET
      end if
    case ("root_fraction_clay%value")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_clay'"
        return
      end if
      if (ieee_is_nan(this%root_fraction_clay%value)) status = NML_ERR_NOT_SET
    case ("root_fraction_clay%optimize")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_clay'"
        return
      end if
    case ("root_fraction_clay%lower_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_clay'"
        return
      end if
    case ("root_fraction_clay%upper_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_clay'"
        return
      end if
    case ("root_fraction_clay")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'root_fraction_clay'"
        return
      end if
      if (ieee_is_nan(this%root_fraction_clay%value)) then
        status = NML_ERR_NOT_SET
      end if
    case ("field_capacity_min%value")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_min'"
        return
      end if
      if (ieee_is_nan(this%field_capacity_min%value)) status = NML_ERR_NOT_SET
    case ("field_capacity_min%optimize")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_min'"
        return
      end if
    case ("field_capacity_min%lower_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_min'"
        return
      end if
    case ("field_capacity_min%upper_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_min'"
        return
      end if
    case ("field_capacity_min")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_min'"
        return
      end if
      if (ieee_is_nan(this%field_capacity_min%value)) then
        status = NML_ERR_NOT_SET
      end if
    case ("field_capacity_delta%value")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_delta'"
        return
      end if
      if (ieee_is_nan(this%field_capacity_delta%value)) status = NML_ERR_NOT_SET
    case ("field_capacity_delta%optimize")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_delta'"
        return
      end if
    case ("field_capacity_delta%lower_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_delta'"
        return
      end if
    case ("field_capacity_delta%upper_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_delta'"
        return
      end if
    case ("field_capacity_delta")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'field_capacity_delta'"
        return
      end if
      if (ieee_is_nan(this%field_capacity_delta%value)) then
        status = NML_ERR_NOT_SET
      end if
    case ("jarvis_sm_threshold%value")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'jarvis_sm_threshold'"
        return
      end if
      if (ieee_is_nan(this%jarvis_sm_threshold%value)) status = NML_ERR_NOT_SET
    case ("jarvis_sm_threshold%optimize")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'jarvis_sm_threshold'"
        return
      end if
    case ("jarvis_sm_threshold%lower_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'jarvis_sm_threshold'"
        return
      end if
    case ("jarvis_sm_threshold%upper_bound")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'jarvis_sm_threshold'"
        return
      end if
    case ("jarvis_sm_threshold")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'jarvis_sm_threshold'"
        return
      end if
      if (ieee_is_nan(this%jarvis_sm_threshold%value)) then
        status = NML_ERR_NOT_SET
      end if
    case default
  ...
  end function nml_soil_moisture_3_is_set

  !> \brief Validate required values and constraints
  integer function nml_soil_moisture_3_is_valid(this, errmsg) result(status)
    class(nml_soil_moisture_3_t), intent(in) :: this !< namelist instance
    character(len=*), intent(out), optional :: errmsg !< error message for non-OK status values
    integer :: istat
    ...
    istat = this%is_set("root_fraction_sand", errmsg=errmsg)
    if (istat == NML_ERR_NOT_SET) then
      status = NML_ERR_REQUIRED
      if (present(errmsg)) then
        if (len_trim(errmsg) == 0) then
          errmsg = "field not set: root_fraction_sand"
        end if
        errmsg = "required " // trim(errmsg)
      end if
      return
    ...
    istat = this%is_set("root_fraction_clay", errmsg=errmsg)
    if (istat == NML_ERR_NOT_SET) then
      status = NML_ERR_REQUIRED
      if (present(errmsg)) then
        if (len_trim(errmsg) == 0) then
          errmsg = "field not set: root_fraction_clay"
        end if
        errmsg = "required " // trim(errmsg)
      end if
      return
    ...
    istat = this%is_set("field_capacity_min", errmsg=errmsg)
    if (istat == NML_ERR_NOT_SET) then
      status = NML_ERR_REQUIRED
      if (present(errmsg)) then
        if (len_trim(errmsg) == 0) then
          errmsg = "field not set: field_capacity_min"
        end if
        errmsg = "required " // trim(errmsg)
      end if
      return
    ...
    istat = this%is_set("field_capacity_delta", errmsg=errmsg)
    if (istat == NML_ERR_NOT_SET) then
      status = NML_ERR_REQUIRED
      if (present(errmsg)) then
        if (len_trim(errmsg) == 0) then
          errmsg = "field not set: field_capacity_delta"
        end if
        errmsg = "required " // trim(errmsg)
      end if
      return
    ...
    istat = this%is_set("jarvis_sm_threshold", errmsg=errmsg)
    if (istat == NML_ERR_NOT_SET) then
      status = NML_ERR_REQUIRED
      if (present(errmsg)) then
        if (len_trim(errmsg) == 0) then
          errmsg = "field not set: jarvis_sm_threshold"
        end if
        errmsg = "required " // trim(errmsg)
      end if
      return
  ...
  end function nml_soil_moisture_3_is_valid

end module nml_soil_moisture_3

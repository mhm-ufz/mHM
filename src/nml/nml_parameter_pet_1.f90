!> \file nml_parameter_pet_1.f90
!> \copydoc nml_pet_1

!> \brief PET - Case 1
!> \details Parameters for the Hargreaves-Samani PET method.
!> \version 0.2
...
module nml_pet_1
  ...
  type, public :: nml_pet_1_t
    ...
    type(parameter_t) :: hargreaves_samani_coefficient !< Hargreaves-Samani coefficient
  contains
    procedure :: init => nml_pet_1_init
    procedure :: init_type => nml_pet_1_init_type
    procedure :: from_file => nml_pet_1_from_file
    procedure :: set => nml_pet_1_set
    procedure :: is_set => nml_pet_1_is_set
    procedure :: is_valid => nml_pet_1_is_valid
  end type nml_pet_1_t

contains

  !> \brief Initialize defaults and sentinels for pet_1
  integer function nml_pet_1_init(this, errmsg) result(status)
    class(nml_pet_1_t), intent(inout) :: this !< namelist instance
    ...
    status = this%init_type( &
      ...
      hargreaves_samani_coefficient=this%hargreaves_samani_coefficient, &
      errmsg=errmsg)
    if (status /= NML_OK) return
  end function nml_pet_1_init

  !> \brief Initialize derived values with their field-specific defaults
  integer function nml_pet_1_init_type(this, &
    correction_factor_min, &
    correction_factor_delta, &
    aspect_threshold, &
    hargreaves_samani_coefficient, &
    errmsg) result(status)
    class(nml_pet_1_t), intent(in) :: this !< parent namelist instance
    ...
    type(parameter_t), intent(inout), optional :: hargreaves_samani_coefficient !< Hargreaves-Samani coefficient
    character(len=*), intent(out), optional :: errmsg !< error message for non-OK status values
  ...
  !> \brief Read pet_1 namelist from file
  integer function nml_pet_1_from_file(this, file, errmsg) result(status)
    class(nml_pet_1_t), intent(inout) :: this !< namelist instance
    ...
    type(parameter_t) :: hargreaves_samani_coefficient
    ! locals
    type(nml_file_t) :: nml
    integer :: iostat
    ...
    status = nml%find("pet_1", errmsg=errmsg)
    ...
    ! read namelist
    read(nml%unit, nml=pet_1, iostat=iostat, iomsg=iomsg)
    if (iostat /= 0) then
      status = NML_ERR_READ
    ...
    this%hargreaves_samani_coefficient = hargreaves_samani_coefficient

    ! mark as configured
    this%is_configured = .true.
    status = NML_OK
  end function nml_pet_1_from_file

  !> \brief Set pet_1 values
  integer function nml_pet_1_set(this, &
    correction_factor_min, &
    correction_factor_delta, &
    aspect_threshold, &
    hargreaves_samani_coefficient, &
    ...
    type(parameter_t), intent(in) :: hargreaves_samani_coefficient !< Hargreaves-Samani coefficient
    ...
    this%is_configured = .true.
    status = NML_OK
  end function nml_pet_1_set

  !> \brief Check whether a namelist value was set
  integer function nml_pet_1_is_set(this, name, idx, errmsg) result(status)
    class(nml_pet_1_t), intent(in) :: this !< namelist instance
    ...
    case ("hargreaves_samani_coefficient%value")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'hargreaves_samani_coefficient'"
        return
      end if
      if (ieee_is_nan(this%hargreaves_samani_coefficient%value)) status = NML_ERR_NOT_SET
    case ("hargreaves_samani_coefficient%optimize")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'hargreaves_samani_coefficient'"
        return
    ...
    case ("hargreaves_samani_coefficient")
      if (present(idx)) then
        status = NML_ERR_INVALID_INDEX
        if (present(errmsg)) errmsg = "index not supported for 'hargreaves_samani_coefficient'"
        return
      end if
      if (ieee_is_nan(this%hargreaves_samani_coefficient%value)) then
        status = NML_ERR_NOT_SET
      end if
    case default
  ...
  end function nml_pet_1_is_set

  !> \brief Validate required values and constraints
  integer function nml_pet_1_is_valid(this, errmsg) result(status)
    class(nml_pet_1_t), intent(in) :: this !< namelist instance
    character(len=*), intent(out), optional :: errmsg !< error message for non-OK status values
    integer :: istat
    ...
    istat = this%is_set("hargreaves_samani_coefficient", errmsg=errmsg)
    if (istat == NML_ERR_NOT_SET) then
      status = NML_ERR_REQUIRED
      if (present(errmsg)) then
        if (len_trim(errmsg) == 0) then
          errmsg = "field not set: hargreaves_samani_coefficient"
        end if
        errmsg = "required " // trim(errmsg)
      end if
      return
  ...
  end function nml_pet_1_is_valid

end module nml_pet_1
